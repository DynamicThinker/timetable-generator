...
export async function generateTimetable({
  ...
  try {
    ...
    const isSlotAvailable = (roomId: string, facultyId: string | null, day: string, timeSlotId: string): boolean => {
    ...
    const bookSlot = (roomId: string, facultyId: string | null, day: string, timeSlotId: string) => {
      const key = `${day}_${timeSlotId}`
    ...
    for (let i = 0; i < sortedSections.length; i++) {
      ...
      for (const day of DAYS) {
        ...
        for (const timeSlot of timeSlots) {
          ...
          for (const room of suitableRooms) {
            if (isSlotAvailable(room.id, section.faculty_id, day, timeSlot.id)) {
      ...
      onProgress?.(progress, `Scheduled ${i + 1}/${sortedSections.length} sections...`)
    }
    ...
    if (schedule.length > 0) {
      const { error: insertError } = await supabase.from("timetable_entries").insert(schedule)
